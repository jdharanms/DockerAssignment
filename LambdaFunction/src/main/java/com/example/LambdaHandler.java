...
public class LambdaHandler implements RequestHandler<S3Event, String> {
    ...
    Logger logger = LoggerFactory.getLogger(com.example.LambdaHandler.class);
    ...
    public String handleRequest(S3Event event, Context context) {
        ...
        AmazonS3 s3Client = AmazonS3ClientBuilder.standard().withEndpointConfiguration(new AwsClientBuilder.EndpointConfiguration("http://s3.us-east-1.localhost.localstack.cloud:4566", "us-east-1")).build();
        ...
        try (Scanner scanner = new Scanner(s3ObjectWrapper.getObjectContent())) {
            ...
            String[] jsonLines = data.split("\n");
            ObjectMapper mapper = new ObjectMapper();
            CollectionType listType = mapper.getTypeFactory().constructCollectionType(List.class, Price.class);
            List<Price> priceList = mapper.readValue("[" + String.join(",", jsonLines) + "]", listType);
